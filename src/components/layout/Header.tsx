...
export default function Header() {
  ...
  return (
    <nav className="navbar bg-base-300 relative z-40 w-full">
      <div className="flex-none">
        {/* Only show the hamburger menu on mobile IF they are logged in AND on the dashboard */}
            ...
            <div className="dropdown dropdown-end">
              ...
              >
                <li className="pointer-events-none mb-2 flex flex-col items-start gap-0 px-4 py-2 text-xs opacity-50">
                  <span className="text-base-content w-full truncate text-sm font-bold">
                    {user.displayName || 'User'}
                  </span>
                  <span className="w-full truncate">{user.email}</span>
                </li>
                {/* Add a quick link to dashboard in the dropdown if they are on the home page */}
                {!isDashboardRoute && (
                  <li>
